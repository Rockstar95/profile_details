...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> signInWithGoogle() async {
    ...
    if (user != null) {
      onSuccess(user, createAccountIfNotExist: true);
    }
    else {
      setState(() {
        isLoading = false;
  ...
  Future<void> signInWithEmailAndPassword() async {
    ...
    User? user = await AuthenticationController().signInWithEmailAndPassword(context, email: emailController.text.trim(), password: passwordController.text.trim());
  ...
  Future<void> sendPasswordResetLink() async {
    ...
    bool isPasswordResetemailSent = await AuthenticationController().sendPasswordResetMail(context, emailController.text);

    if(isPasswordResetemailSent) {
      Snakbar().show_success_snakbar(context, "Password reset email sent");
    }
    else {
  ...
  Future<void> onSuccess(User user, {bool createAccountIfNotExist = false}) async {
  ...
  Widget getForgotPasswordLinkText() {
    return Container(
      margin: EdgeInsets.only(left: MySize.size48!, right: MySize.size48!, top: 10),
      child: InkWell(
        onTap: () async {
  ...
  Widget getRegitrationLinkText() {
    return Container(
      margin: EdgeInsets.only(left: MySize.size48!, right: MySize.size48!, top: MySize.size40!),
      child: InkWell(
        onTap: () {
          Navigator.pushNamedAndRemoveUntil(context, RegistrationScreen.routeName, (_) => false);
        },
        child: Text(
          "New User?",
          softWrap: true,
